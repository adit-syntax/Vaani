...
const Login = () => {
  const { authStep, state, setAuthStep } = useVaani();
  ...
  const runLivenessCheck = async () => {
    ...
    try {
      ...
      if (livenessResult.success) {
        ...
        // UID is set in window.vaani_uid by Context after Step 1
        const currentUid = (window as any).vaani_uid;
        if (!currentUid) {
        ...
        const anchorBase64 = localStorage.getItem(`vaani_face_${currentUid}`);
        if (!anchorBase64) {
