...
describe("Auth Pipeline Boundary Isolation", () => {
  ...
  test("voiceStateController exports NO auth-related functions", async () => {
    const voiceController = await import("@/lib/vaani/voiceStateController")
  ...
  test("voiceStateController PauseReason has NO auth-specific values", async () => {
    const { pauseListening, resumeListening } = await import("@/lib/vaani/voiceStateController")
...
describe("Voice-First Registration Flow", () => {
  ...
  test("step transitions follow correct sequence", () => {
    ...
    expect(getNextStep("PASSWORD")).toBe("CONFIRM_PASSWORD")
    expect(getNextStep("CONFIRM_PASSWORD")).toBe("APP_PASSWORD")
    expect(getNextStep("APP_PASSWORD")).toBe("CONFIRM_APP_PASSWORD")
    expect(getNextStep("CONFIRM_APP_PASSWORD")).toBe("FACE")
    expect(getNextStep("VOICE_PIN")).toBe("CONFIRM_VOICE_PIN")
    expect(getNextStep("CONFIRM_VOICE_PIN")).toBe("COMPLETE")
  })
  ...
  test("previous step transitions are correct", () => {
    ...
    expect(getPreviousStep("FACE")).toBe("CONFIRM_APP_PASSWORD")
  })
