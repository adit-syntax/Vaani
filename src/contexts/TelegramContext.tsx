...
export const TelegramProvider = ({ children }: { children: ReactNode }) => {
    ...
    useEffect(() => {
        try {
            const stored = localStorage.getItem("vaani_telegram_history");
            if (stored) {
    ...
    useEffect(() => {
        if (Object.keys(history).length > 0) {
            localStorage.setItem("vaani_telegram_history", JSON.stringify(history));
        }
    }, [history]);
            ...
            const unsubSnap = onSnapshot(q, (snap) => {
                ...
                if (settings.messageAlerts && newMessages.length > 0) {
                    if (newMessages.length === 1) {
                        ...
                        if (upd.senderId !== 0) {
                            window.dispatchEvent(new CustomEvent("vaani:notify", {
