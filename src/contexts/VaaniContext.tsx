...
import { detectIntent, TargetPlatform } from "@/lib/vaani/intentMap";
...
import { unlockAudio } from "@/services/audioSoundService";
import { useSettings } from "@/contexts/SettingsContext";
import type { VaaniState } from "@/lib/vaani/stateMachine";
import { handleGlobalCommand } from "@/lib/vaani/commandRouter";
...
import {
  canAcceptVoice,
} from "@/lib/vaani/stateMachine";
import { useNavigate } from "react-router-dom";
import { resetVoiceController, pauseListening, stopListening, resumeListening, startListening, requestAudioPermission } from "@/lib/vaani/voiceStateController";
import {
  restoreSession,
  createSession,
  destroySession,
...
interface VaaniContextType {
  ...
  setState: React.Dispatch<React.SetStateAction<VaaniState>>;
  wakeUp: () => void;
...
const VaaniContext = createContext<VaaniContextType | undefined>(undefined);


export const useVaani = () => {
  const ctx = useContext(VaaniContext);
  if (!ctx) throw new Error("VaaniContext missing");
  return ctx;
};
...
export const VaaniProvider = ({ children }: { children: ReactNode }) => {
  const settings = useSettings();
  const gmail = useGmail();
  const telegram = useTelegram();
  const navigate = useNavigate();
  const [state, setState] = useState<VaaniState>("DORMANT");
  ...
  useEffect(() => {
    const saved = localStorage.getItem("vaani_command_count");
    if (saved) setCommandCount(parseInt(saved));
  }, []);

  useEffect(() => {
    localStorage.setItem("vaani_command_count", commandCount.toString());
  }, [commandCount]);
  ...
  const stateRef = useRef<VaaniState>("DORMANT");
  ...
  useEffect(() => {
    ...
    window.addEventListener("vaani:voice", authHandler);
    return () => window.removeEventListener("vaani:voice", authHandler);
  }, []);
  ...
  useEffect(() => {
    const handleVoiceEvent = (e: any) => {
      ...
      switch (event.type) {
        ...
        case "ERROR":
          ...
          addMessage(
            "system",
            `Microphone Error (${event.reason}): Unable to capture audio. Please ensure your microphone is connected, unmuted, and allowed in browser settings, then click the mic button to retry.`
          );
          break;
      }
    };

    window.addEventListener("vaani:voice_event", handleVoiceEvent);
    ...
    window.addEventListener("vaani:wake", onWake);
    window.addEventListener("vaani:sleep", onSleep);

    return () => {
      window.removeEventListener("vaani:voice_event", handleVoiceEvent);
      window.removeEventListener("vaani:wake", onWake);
      window.removeEventListener("vaani:sleep", onSleep);
    };
  }, []); // Dependencies for closure safety

  /* ================= REAL-TIME INTERIM SPEECH LISTENER ================= */
  useEffect(() => {
    ...
    window.addEventListener("vaani:interim_transcript", handleInterim);
    return () => window.removeEventListener("vaani:interim_transcript", handleInterim);
  }, []);
  ...
  useEffect(() => {
    ...
    window.addEventListener("vaani:wake", onWakeStateRestore);

    return () => {
      window.removeEventListener("vaani:wake", onWakeStateRestore);
    };
  }, []);
  ...
  useEffect(() => {
    ...
    window.addEventListener("vaani:face", onFaceEvent);
    return () => window.removeEventListener("vaani:face", onFaceEvent);
  }, []);
    ...
    if (authMode === "REGISTER" && authStep === "COMPLETE") {
      (async () => {
        try {
          ...
          const tempAnchor = sessionStorage.getItem("vaani_temp_anchor");
          if (tempAnchor) {
            localStorage.setItem(`vaani_face_${regResult.uid}`, tempAnchor);
            sessionStorage.removeItem("vaani_temp_anchor");
          ...
          speak("Registration successful. Welcome to Vaani! You are now logged in and verified.");
    ...
    if (authMode === "LOGIN" && authStep === "FACE") {
      (async () => {
        ...
        if (result.status === "BIOMETRIC_REQUIRED") {
          ...
          (window as any).vaani_uid = result.user.uid; // 🔥 Link UID for local face match
  ...
  const enableAssistant = async () => {
    ...
    // 🔑 Unlock audio system & request microphone stream permission
    warmUpTTS();
    unlockAudio();
    ...
    if (micGranted) {
      setState("LISTENING");
      startListening();
      speak("Vaani voice assistant is now enabled.");
    } else {
      setState("ERROR");
      addMessage(
        "system",
        "Microphone access permission was not granted. Please allow microphone access in your browser settings to use voice features."
      );
    }
  };
  ...
  const speak = (text: string, options?: { cancelPrevious?: boolean, volume?: number, rate?: number }) => {
    ...
    if (!settings.voiceFeedback) {
      console.log(`[VAANI FEEDBACK (SILENT)]: ${text}`);
      addMessage("assistant", text);
      return;
    }

    console.log(`[VAANI SPEAKING]: ${text}`);
    addMessage("assistant", text);
    speakText(text, {
      ...options,
  ...
  useEffect(() => {
    const handleNotify = (e: any) => {
      if (assistantEnabled) {
        ...
        console.log("[VAANI] Notification queued (waiting for assistant enable):", e.detail.text);
      }
    };

    window.addEventListener("vaani:notify", handleNotify);
    return () => window.removeEventListener("vaani:notify", handleNotify);
  }, [assistantEnabled]);
  ...
  const handleIntent = async (text: string) => {
    ...
    if (intent.platform !== "system") {
      // 🔒 AUTH GUARD: Require login/registration before accessing Gmail, Telegram, or other platforms
      if (!isAuthenticated) {
        speak("Please log in or register first to access your Gmail and personal tools.", { cancelPrevious: true });
        setRouteIntent("/login");
        return;
      }
