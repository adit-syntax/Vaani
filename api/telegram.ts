...
async function getFirebaseAdmin() {
    ...
    const existing = admin.apps.find(a => a?.name === 'vaani-prod');
    if (existing) {
    ...
    try {
        if (saKeyEnv) {
            ...
            firebaseApp = admin.initializeApp({ credential: admin.credential.cert(config) }, 'vaani-prod');
        } else {
            firebaseApp = admin.initializeApp({ projectId: pId }, 'vaani-prod');
        }
        return firebaseApp;
    } catch (fatal: any) {
        console.error("🛑 [TELEGRAM FB ADMIN] Init Failure:", fatal.message);
        if (fatal.code === 'app/duplicate-app') return admin.app('vaani-prod');
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
    ...
    const host = req.headers.host || 'vaaniai.vercel.app';
